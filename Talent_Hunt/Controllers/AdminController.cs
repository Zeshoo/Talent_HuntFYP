...
namespace Talent_Hunt.Controllers
{
    public class AdminController : Controller
    {
        ...
        public async Task<ActionResult> LeaderBoard()
        {
            using (HttpClient client = new HttpClient())
            {
                client.BaseAddress = new Uri("http://localhost/TalentHunt1/"); // Replace with your API base URL

                HttpResponseMessage response = await client.GetAsync("api/Main/LeaderBoard");

                if (response.IsSuccessStatusCode)
                {
                    var json = await response.Content.ReadAsStringAsync();
                    var result = JsonConvert.DeserializeObject<List<LeaderBoard>>(json);
                    return View(result);
                }
                else
                {
                    ViewBag.Error = "Failed to load toppers.";
                    return View(new List<LeaderBoard>());
                }
            }
        }
        [HttpGet]
        ...
        public async Task<ActionResult> ViewSubmissions(int taskId)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                if (taskResponse.IsSuccessStatusCode)
                {
                    ...
                    // Manually parse JSON to get title & description
                    dynamic taskData = JsonConvert.DeserializeObject(taskJson);
                    model.EventTitle = taskData?.EventTitle;
                    model.Description = taskData?.Description;
                }
                else
                {
                ...
                if (subResponse.IsSuccessStatusCode)
                {
                    ...
                    // ✅ This is where you should place the corrected line
                    model.Submissions = JsonConvert.DeserializeObject<List<SubmissionDetail>>(subJson);
                }
                else
                {
                    model.Submissions = new List<SubmissionDetail>();
